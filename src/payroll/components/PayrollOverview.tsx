...
export const PayrollOverview: React.FC<PayrollOverviewProps> = ({
  ...
  // Compute total in memory strictly for local inspection if revealed
  const totalAmount = currentBatch 
    ? currentBatch.recipients.reduce((sum, r) => sum + r.paymentAmount, 0)
    : 48320;

  const activeEmployees = currentBatch ? currentBatch.recipients.length : 14;
  const nextPayroll = currentBatch ? currentBatch.paymentDate : '30 SEP 2026';
  const payrollStatus = currentBatch ? currentBatch.status : 'READY';
    ...
    <div className="space-y-6 text-left font-sans">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div className="p-4 bg-[#0E0E0E] border border-white/[0.08] space-y-1">
          ...
          <div className="text-2xl sm:text-3xl font-condensed font-extrabold text-[#E8E6DF]">
            {activeEmployees}
          </div>
        ...
        {/* Metric 2: Next Payroll */}
        <div className="p-4 bg-[#0E0E0E] border border-white/[0.08] space-y-1">
          <div className="text-[10px] font-mono tracking-wider text-[#8A8882] uppercase flex items-center justify-between">
            <span>NEXT PAYROLL</span>
            <Calendar className="w-3.5 h-3.5 text-[#8A8882]" />
          </div>
          <div className="text-xl sm:text-2xl font-condensed font-extrabold text-[#E8E6DF] truncate">
            {nextPayroll}
          </div>
          <div className="text-[10px] font-mono text-[#8A8882]">
            BATCH CYCLE
        ...
        {/* Metric 3: Payroll Status */}
        <div className="p-4 bg-[#0E0E0E] border border-white/[0.08] space-y-1">
          <div className="text-[10px] font-mono tracking-wider text-[#8A8882] uppercase flex items-center justify-between">
            <span>PAYROLL STATUS</span>
            <span className="w-2 h-2 rounded-full bg-[#26A17B] animate-pulse"></span>
          </div>
          <div className="text-2xl sm:text-3xl font-condensed font-extrabold text-[#26A17B]">
            {payrollStatus}
          </div>
          <div className="text-[10px] font-mono text-[#8A8882]">
            PRE-AUTHORIZED
        ...
        {/* Metric 5: Total Payroll (Protected) */}
        <div className="p-4 bg-[#0E0E0E] border border-white/[0.08] space-y-1 col-span-2 sm:col-span-1">
          ...
          <div className="text-xl sm:text-2xl font-condensed font-extrabold text-[#E8E6DF] tracking-wider">
            {isRevealedLocally ? (
              ...
              <span className="text-[#8A8882] tracking-[0.2em]">
                £••••••••
              </span>
            )}
          </div>
      ...
      {/* Cryptographic Architecture Invariant Banner */}
      <div className="p-3 bg-[#0C0C0C] border border-white/[0.06] text-[11px] font-mono text-[#8A8882] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span>
            <strong className="text-[#E8E6DF]">LOCAL DATA MINIMIZATION:</strong> Revealing amounts locally only renders in-memory browser state. Raw compensation numbers are never submitted to public ledger state.
          </span>
        </div>
        <div className="text-[10px] text-[#8A8882]/70 uppercase shrink-0">
