...
import { revalidateBlackoutSafeWalletSession, type SafeWalletSession } from './wallet-session.ts';
...
const fromHex = (value: string): Uint8Array => {
  ...
  if (!/^[0-9a-f]{2,}$/i.test(hex) || hex.length % 2 !== 0) throw new Error('BLACKOUT_SAFE_INVALID_SERIALIZED_TX');
...
export async function buildBlackoutSafeProviders(session: SafeWalletSession) {
  ...
  const walletProvider: WalletProvider = {
    ...
    async balanceTx(tx) {
      try {
        ...
        if (!result || typeof result.tx !== 'string' || result.tx.length === 0) throw new Error('BLACKOUT_SAFE_BALANCER_RETURNED_NO_TX');
