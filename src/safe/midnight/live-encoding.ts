...
export function bytesToHex32(value: Uint8Array): Hex32String {
  if (!(value instanceof Uint8Array) || value.length !== 32) throw new Error('BLACKOUT_SAFE_HEX32_REQUIRES_32_BYTES');
...
export function exactGovernanceBytesCommitment(safeId: Uint8Array, action: string, value: Uint8Array): Uint8Array {
  if (safeId.length !== 32) throw new Error('BLACKOUT_SAFE_SAFE_ID_MUST_BE_32_BYTES');
...
export function createMembershipSetup(memberCount: number, safeId = randomBytes32()): MembershipSetup {
  ...
  const kits = secrets.map((secret, leafIndex): SerializedSafeSignerKit => {
    ...
    for (let level = 0; level < MEMBER_TREE_DEPTH; level += 1) {
      ...
      path.push({ sibling: fieldToHex32(levels[level][siblingIndex]), goes_left: index % 2 === 0 });
    ...
    return {
      ...
      memberPath: {
        is_some: true,
        value: { leaf: bytesToHex32(commitments[leafIndex]), path },
      },
    };
  });
