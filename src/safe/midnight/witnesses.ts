...
function resolve<TArg, TResult>(
  ...
  return typeof value === 'function' ? (value as (item: TArg) => TResult)(arg) : value;
}
...
export function buildBlackoutSafeWitnesses(
  ...
  return {
    local_member_secret: (context) => [context.privateState, required('MEMBER_SECRET', bundle.memberSecret)],
    local_member_path: (context, memberCommitment) => [context.privateState, resolve('MEMBER_PATH', bundle.memberPath, memberCommitment)],
    local_private_proposal: (context) => [context.privateState, required('PRIVATE_PROPOSAL', bundle.privateProposal)],
    local_policy: (context) => [context.privateState, required('POLICY', bundle.policy)],
    local_next_policy: (context) => [context.privateState, required('NEXT_POLICY', bundle.nextPolicy)],
    held_coin: (context, color) => [context.privateState, resolve('HELD_COIN', bundle.heldCoin, color)],
  } as BlackoutSafeWitnesses;
}
