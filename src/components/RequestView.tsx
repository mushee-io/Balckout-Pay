...
import { Plus, Copy, Check, Search, Download } from 'lucide-react';
import { VerificationRequest } from '../midnight/types';
import { exportPublicLiveEvidence } from '../midnight/evidence-store';
...
export const RequestView: React.FC<RequestViewProps> = ({
  ...
  const hasLiveEvidence = requests.some((req) => req.isLiveOnChain);
  ...
  const handleExportEvidence = () => {
    const payload = exportPublicLiveEvidence();
    const blob = new Blob([payload], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `blackout-midnight-preview-evidence-${Date.now()}.json`;
    ...
    <div className="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-12 py-10 space-y-12 text-left font-sans select-none">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 pb-6 border-b border-white/[0.08]">
        <div className="space-y-2">
          <div className="text-[11px] font-mono tracking-[0.25em] text-[#FF5A5F] uppercase font-bold">[ VERIFICATION REQUESTS PORTAL ]</div>
          <h1 className="text-3xl sm:text-5xl font-condensed font-extrabold uppercase text-[#E8E6DF]">REQUEST ELIGIBILITY PROOF.</h1>
        ...
        <div className="flex items-center gap-2">
          {hasLiveEvidence && (
            <button
              ...
              className="px-4 py-3 bg-[#0E0E0E] text-[#26A17B] border border-[#26A17B]/50 hover:border-[#26A17B] font-mono text-[11px] font-bold uppercase tracking-[0.12em] flex items-center gap-2 cursor-pointer"
              title="Export public-only PASS/FAIL transaction evidence"
            >
        ...
        <div className="flex items-center gap-1 font-mono text-xs uppercase">
          {['ALL', 'PENDING', 'VERIFIED', 'REJECTED'].map((st) => (
            <button
              ...
              className={`px-3 py-2 transition-all cursor-pointer ${filterStatus === st ? 'bg-[#E8E6DF] text-black font-bold' : 'bg-[#141414] text-[#8A8882] hover:text-[#E8E6DF]'}`}
            >
              {st}
            </button>
          ))}
                ...
                return (
                  <tr key={req.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="py-4 px-6">
                      <div className="font-bold text-[#E8E6DF] text-sm font-sans uppercase">{req.title}</div>
                      <div className="text-[10px] text-[#8A8882] flex items-center gap-2 mt-0.5">
                        <span>{req.purpose}</span><span>•</span><span className="text-[#8A8882]/80">{req.id}</span>
                      </div>
                      {req.proofResult?.txHash && (
                        <div className="mt-1 text-[9px] text-[#26A17B]">
                          TX {req.proofResult.txHash.slice(0, 12)}... · BLOCK {req.proofResult.blockHeight ?? 'PENDING'}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="py-4 px-6 font-bold text-[#FF5A5F]">Income ≥ £{req.requiredIncome.toLocaleString()}/mo</td>
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {hasProof ? (
                          <button onClick={() => onSelectRequestToView(req)} className="px-3 py-1.5 bg-[#181818] hover:bg-[#E8E6DF] hover:text-black border border-white/[0.1] text-[#E8E6DF] font-mono text-[11px] uppercase tracking-wider transition-all cursor-pointer">VIEW RECEIPT →</button>
                        ) : (
                          <button onClick={() => onSelectRequestToProve(req)} className="px-3 py-1.5 bg-[#181818] hover:bg-[#FF5A5F] hover:text-black border border-white/[0.1] text-[#FF5A5F] font-mono text-[11px] uppercase tracking-wider transition-all cursor-pointer font-bold">PROVE NOW →</button>
                        )}
                        <button
                          onClick={() => handleCopy(req.proofResult?.txHash || req.id, req.id)}
                          ...
                          title={req.proofResult?.txHash ? 'Copy transaction ID' : 'Copy request ID'}
                        >
      ...
      <div className="p-6 bg-[#0E0E0E] border border-white/[0.08] space-y-4 font-mono text-xs">
        ...
        <div className="p-4 bg-[#090909] border border-white/[0.06] text-[#8A8882] space-y-1">
          ...
          <div className="text-[#E8E6DF]">&lt;button onClick=&#123;() =&gt; blackout.requestProof(&#123; threshold: 2500 &#125;)&#125;&gt;PROVE INCOME PRIVATELY&lt;/button&gt;</div>
        </div>
      </div>
    </div>
  );
